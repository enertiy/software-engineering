...
class VariableSaver(VarScopeObject):
    ...
    def __init__(self, variables, save_dir, max_versions=2,
        ...
        Args:
            ...
            max_versions (int): Maximum versions to keep in the directory
                ...
                prevent corrupted latest files caused by IO failure.
            filename (str): Name of the files of variable values (default is
                ``variables.dat``).
            latest_file (str): Name of the file which organizes the latest
    ...
    def restore(self, ignore_non_exist=False):
        """
        Restore the latest from file if it exists.

        Args:
            ignore_non_exist (bool): Whether or not to ignore error if the
                latest file does not exist? (default :obj:`False`)

        Raises:
            IOError: If the latest files do not exist, and
        ...
        if file_path:
            ...
            getLogger(__name__).debug(
                'Restored from latest file %r.', file_path)
